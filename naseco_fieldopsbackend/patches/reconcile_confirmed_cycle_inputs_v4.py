...
def execute():
    """Create reviewable requests when legacy or exposure checks block approval."""
    ...
    for index, cycle_name in enumerate(cycles):
        savepoint = f"reconcile_cycle_inputs_v4_{index}"
        ...
        try:
            ...
            frappe.log_error(
                title=f"Input reconciliation v4 failed for {cycle_name}",
