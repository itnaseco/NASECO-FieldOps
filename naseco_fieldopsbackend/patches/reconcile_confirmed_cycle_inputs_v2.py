...
def execute():
    """Retry legacy confirmed cycles after Active-recipe compatibility support."""
    ...
    for index, cycle_name in enumerate(cycles):
        savepoint = f"reconcile_cycle_inputs_v2_{index}"
        ...
        try:
            ...
            frappe.log_error(
                title=f"Input reconciliation v2 failed for {cycle_name}",
